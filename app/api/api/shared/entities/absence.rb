class API::Shared::Entities::Absence < Grape::Entity
  ...
  expose :weekly_repeat_interval, if: ->(absence, options) { absence.recurring? } do |absence, options|
    absence.schedule.weekly_repeat_interval
  end
  expose :daypart
  ...
  def embed_time_type?(absence, options)
    options[:embed] && options[:embed].include?('time_type')
  end

  def embed_user?(absence, options)
